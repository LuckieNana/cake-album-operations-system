...
def maybe_restart_for_static_serving():
    """Restart this Streamlit process once, automatically, so the static-serving config
    written above actually takes effect. Without this, someone has to remember to
    restart the service by hand before push notifications can ever work.
    Set CAKE_ALBUM_NO_AUTORESTART=1 if you would rather restart it yourself."""
    import os
    import sys
    if static_serving_enabled():
        return
    if os.environ.get("CAKE_ALBUM_NO_AUTORESTART") or os.environ.get("CAKE_ALBUM_STATIC_RESTARTED"):
        print("[PUSH] Static serving is OFF. Restart Streamlit once so push notifications can work.", flush=True)
        return
    os.environ["CAKE_ALBUM_STATIC_RESTARTED"] = "1"
    print("[PUSH] Restarting once to switch on static file serving…", flush=True)
    attempts = []
    # However this process was launched, one of these puts it back with the new config.
    if sys.argv and sys.argv[0]:
        attempts.append((sys.argv[0], list(sys.argv)))
    attempts.append((sys.executable, [sys.executable, "-m", "streamlit"] + list(sys.argv[1:])))
    for program, argv in attempts:
        try:
            os.execv(program, argv)
        except Exception as e:
            print(f"[PUSH] Restart via {program} failed: {e}", flush=True)
    print("[PUSH] Automatic restart failed. Please restart the app service once by hand.", flush=True)
...
APP_VERSION = "v1.3 (QC & Packaging Advance Pipeline View)"
...
def order_card(row, extra=None, show_image=True):
    ...
    if disp(row.get("decorator_assigned")) != "—":
        html.append(f"<b>🎨 Decorator:</b> {disp(row.get('decorator_assigned'))}<br>")
        ...
        if row is not None:
            ...
            if st.button("✅ Piling Complete → Send to Covering Check", width='stretch', disabled=not may_act):
                ...
                create_notification(row.order_id, "Studio / Final QC", None,
                                     f"📥 ADVANCE COPY — {row.order_id} ({disp(row.get('customer_name'))}) has left Production (Piling) and is now going into Covering. Due {disp(row.get('due_date'))}. Prepare packaging for it in 'Incoming Pipeline'.")
                st.rerun()
    with t3:
...
def render_qc_incoming_pipeline(df):
    """A read-only advance copy of every order still upstream in production, so Final QC and
    Packaging can see what is coming, pre-print delivery notes, and pull boxes/ribbons/toppers
    ready BEFORE the cake physically arrives. Nothing here is actionable on purpose — work only
    starts once the cake actually reaches Studio Check, which happens in the 'Final QC' tab."""
    st.markdown("#### 📥 Work Coming Your Way")
    st.caption("A live copy of every order still on the production line. Use it to prepare packaging "
               "materials and delivery notes in advance. You can only sign off a cake once it "
               "physically reaches you and appears under 'Final QC'.")

    all_upstream = [s for _, statuses in QC_UPSTREAM_STAGES for s in statuses]
    pipeline = filter_orders(df, all_upstream)
    ready_now = filter_orders(df, ["Studio Check"])

    m1, m2, m3 = st.columns(3)
    m1.metric("In production (coming)", len(pipeline))
    m2.metric("At your bench now", len(ready_now))
    urgent_incoming = int((pipeline["urgency_level"] == "Urgent").sum()) if not pipeline.empty and "urgency_level" in pipeline.columns else 0
    m3.metric("🚨 Urgent incoming", urgent_incoming)

    if pipeline.empty:
        st.success("Nothing upstream right now — the production line is clear.")
        return

    st.markdown("##### By production stage")
    for stage_name, statuses in QC_UPSTREAM_STAGES:
        stage_q = filter_orders(df, statuses)
        if stage_q.empty:
            continue
        near_ready = stage_name in ("Coating / Covering", "Decoration")
        label = f"{'🔥 ' if near_ready else ''}{stage_name} — {len(stage_q)} order(s)"
        with st.expander(label, expanded=near_ready):
            render_queue_table(stage_q, f"In {stage_name}", ["decorator_assigned"])

    st.markdown("##### Packaging prep sheet")
    prep_cols = ["order_id", "customer_name", "product_type", "cake_size", "urgency_level",
                 "due_date", "expected_time", "workflow_status", "delivery_method",
                 "topper_required", "topper_status", "sticker_required", "sticker_status",
                 "decorator_assigned"]
    prep = pipeline.copy()
    if "due_date" in prep.columns:
        prep = prep.sort_values("due_date")
    table(prep, prep_cols)
    st.caption("Toppers and stickers marked as required but not yet Complete are the usual reason a "
               "cake stalls at packaging — chase those before the cake arrives.")
...
def render_studio_qc():
    ...
    incoming_tab, qc_tab, packaging_tab, dispatch_tab, finished_tab = st.tabs(
        ["📥 Incoming Pipeline", "Final QC", "Packaging & Delivery Notes", "Dispatch Planning", "✅ Finished Work"])
    with incoming_tab:
        render_qc_incoming_pipeline(df)
    with qc_tab:
