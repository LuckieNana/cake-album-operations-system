...
MANIFEST_SOURCE = json.dumps({
    ...
    "theme_color": "#4B2A5C",
    "icons": [
        {"src": "/app/static/icon-192.png", "sizes": "192x192", "type": "image/png", "purpose": "any maskable"},
        {"src": "/app/static/icon-512.png", "sizes": "512x512", "type": "image/png", "purpose": "any maskable"},
    ],
}, indent=2)
...
def ensure_push_assets():
    ...
    try:
        ...
        print(f"[PUSH] Could not write static push assets: {e}", flush=True)
...
APP_VERSION = "v1.7 — Release 7.0 (Self-Contained Push Notifications)"
...
(() => {
  ...
  const SW_URL = __SWURL__;
  const MANIFEST_URL = __MANIFESTURL__;
  const ICON_URL = __ICONURL__;
...
def _render_department_notifications_body():
    ...
    notification_html = (
        ...
        .replace("__SWURL__", json.dumps(SERVICE_WORKER_URL))
        .replace("__MANIFESTURL__", json.dumps(MANIFEST_URL))
...
def render_production_planning():
    ...
    if batchable.empty:
        ...
        if st.button("🧮 Create Batch From This Group", width='stretch'):
            ...
            create_notification(None, "Baking", batch_baker,
                                 f"🧮 New batch {batch_number}: {pick_row['total_layers']:g} layers of {pick_row['flavour']} "
                                 f"({pick_row['cake_size_value']:g}\" {pick_row['cake_shape']}) assigned to you.")
