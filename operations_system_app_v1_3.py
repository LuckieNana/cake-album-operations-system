...
APP_VERSION = "v1.3 — Release 4.0 (Stable)"
...
def render_staff_greeting():
    ...
    name = st.session_state.get("staff_name", "").strip()
    if not name:
        return
...
def render_stage_material_planning(stage, row, default_by):
    ...
    action = c.selectbox("Action", ["Used", "Needed", "Request from Procurement"], key=f"mat_{stage}_action")
    if is_size_family:
        qty = st.number_input("Quantity (pieces)", min_value=0.0, step=1.0, key=f"mat_{stage}_qty")
        unit = "pieces"
    else:
        d,e = st.columns(2)
        qty = d.number_input("Quantity / Weight", min_value=0.0, step=0.5, key=f"mat_{stage}_qty")
...
def render_customer_care():
    ...
    flavours = st.text_input("Customer-selected Flavours *", key="nc_flavours")
    if size_mode == "inches":
        a, b = st.columns(2)
...
def render_baking():
    ...
    with t2:
        ...
        if row is not None:
            ...
            can_pass = elapsed is None or elapsed >= required_mins
...
def render_packaging():
    ...
    with t2:
        ...
        if row is not None:
            order_card(row)
            render_stage_material_planning("Packaging", row, "Packaging")
...
def render_sidebar():
    ...
    st.sidebar.write(f"Signed in as: **{st.session_state.get('staff_name','')}**")
