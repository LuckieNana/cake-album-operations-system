...
APP_VERSION = "v1.6 — Release 6.0 (Real Push Notifications)"
...
def _render_department_notifications_body():
    ...
    (() => {{
      ...
      async function subscribeForRealPush() {{
        const vapidPublicKey = {vapid_public_key_json};
        ...
        try {{
          ...
          await fetch("/push/subscribe", {{
            ...
            body: JSON.stringify({{username: username, department: departments, subscription: subscription.toJSON()}})
          }});
        }} catch (e) {{ /* Push setup failing should never break the rest of the page */ }}
      }}
      ...
      refreshControls();
      announceNew();
      if (Notification.permission === "granted") subscribeForRealPush();
    }})();
    </script>
    """
...
VAPID_PUBLIC_KEY = VAPID_PUBLIC_KEY_FILE.read_text().strip() if VAPID_PUBLIC_KEY_FILE.exists() else None
