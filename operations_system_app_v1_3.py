...
APP_VERSION = "v3.2 — Release 21.0 (Chat Photos + Visual Upload Preview + EOD Decoration Materials)"
    ...
    with st.sidebar:
        ...
        with st.expander(header, expanded=(unread_count > 0)):
            ...
            if chat_conv:
                ...
                # IMPORTANT: do not use Streamlit's extension filter for phone photos.
                # iOS/Safari can hand Photos-library images to the browser with a generic
                # filename/MIME combination; Streamlit may then reject the upload in the
                # browser (red caution icon) before our Python code ever sees the file.
                # Accept the file first and validate it ourselves after submit.
                chat_photo = st.file_uploader(
                    "📷 Attach a picture (optional)",
                    key=f"chat_photo_{msg_gen}",
                    help="Works with iPhone and Android photos, including HEIC/HEIF. The app validates and compresses the picture after you send it.",
                )
                chat_photo_ok = True
                chat_photo_reason = ""
                if chat_photo is not None:
                    chat_photo_ok, chat_photo_reason = _looks_like_supported_phone_image(chat_photo)
                    if not chat_photo_ok:
                        st.error(f"This selected file does not look like a supported photo. {chat_photo_reason}")
                    else:
                        try:
                            ...
                            # Upload itself succeeded. Do not turn a preview limitation into
                            # a false upload failure on iPhone. The attachment can still save.
                            st.caption("✅ Photo selected. Preview is unavailable on this phone, but it can still be sent.")

                if st.button("Send", key="chat_send_btn", width='stretch'):
                    if chat_photo is not None and not chat_photo_ok:
                        st.error("Please choose a JPG, PNG, HEIC or HEIF photo.")
                        return
...
def ensure_base_schema():
    ...
    with connect() as conn:
        ...
        conn.execute("""CREATE TABLE IF NOT EXISTS daily_material_accountability(
            id INTEGER PRIMARY KEY AUTOINCREMENT, staff_name TEXT NOT NULL, department TEXT NOT NULL,
            accountability_date TEXT NOT NULL, item_name TEXT NOT NULL, quantity_used REAL NOT NULL,
            unit TEXT NOT NULL, notes TEXT, recorded_at TEXT NOT NULL)""")
...
def ensure_collaboration_schema():
    ...
    with connect() as conn:
        ...
        # Photo attachments for Team Chat / cake conversations. Keep these nullable so
        # every existing comment remains backward compatible.
        safe_add_column(conn, "order_comments", "attachment_filename", "TEXT")
        safe_add_column(conn, "order_comments", "attachment_mime", "TEXT")
        safe_add_column(conn, "order_comments", "attachment_base64", "TEXT")
        conn.commit()
    ...
    with connect() as conn:
        conn.execute("""INSERT INTO order_comments(order_id, author_username, author_name,
                     ...
                     (order_id, author_username, author_name, author_department,
                      ...
                      attachment_filename or "", attachment_mime or "", attachment_base64 or ""))
        conn.commit()
    ...
    with st.expander(label, expanded=expanded):
        ...
        for r in rows:
            ...
            st.markdown(
                ...
                unsafe_allow_html=True)
            # Show an attached clarification/reference photo directly in the cake chat.
            try:
                attachment_b64 = str(r["attachment_base64"] or "") if "attachment_base64" in r.keys() else ""
                if attachment_b64:
                    photo_bytes = base64.b64decode(attachment_b64)
                    st.image(
                        _web_safe_image_payload(photo_bytes, max_dimension=900, jpeg_quality=75),
                        caption=str(r["attachment_filename"] or "Photo") if "attachment_filename" in r.keys() else "Photo",
                        width='stretch')
            except Exception:
        ...
        with st.form(f"{base}_composer_form", clear_on_submit=True):
            ...
            # No Streamlit extension filter: accept the phone photo first, then validate
            # it in Python. This is the same mobile-safe approach used in Customer Care.
            thread_photo = st.file_uploader(
                "📷 Attach Picture / Camera",
                key=f"{base}_photo",
                help="Attach a cake/reference photo from iPhone or Android. JPG, PNG, HEIC and HEIF are supported.")
            cols = st.columns(2)
            post_clicked = cols[0].form_submit_button("💬 Post", width='stretch')
            ai_clicked = cols[1].form_submit_button("🤖 Ask AI", width='stretch')

        if post_clicked:
            if not (msg or "").strip() and thread_photo is None:
                st.warning("Type a message or attach a picture first.")
            else:
                attachment_filename = attachment_mime = attachment_base64 = ""
                if thread_photo is not None:
                    ok, reason = _looks_like_supported_phone_image(thread_photo)
                    if not ok:
                        st.error(f"That file does not look like a supported phone photo. {reason}")
                        return
                    try:
                        photo_bytes, photo_suffix, photo_mime = _prepare_uploaded_reference_image(
                            thread_photo, max_dimension=1200, jpeg_quality=78)
                        attachment_filename = f"{Path(thread_photo.name).stem}{photo_suffix}"
                        attachment_mime = photo_mime
                        attachment_base64 = base64.b64encode(photo_bytes).decode()
                    except Exception as exc:
                        st.error(f"Could not attach this picture: {exc}")
                        return
                saved_message = (msg or "").strip() or "📷 Photo"
                mentioned = add_order_comment(
                    order_id, saved_message,
...
def render_team_hub():
    ...
    with tab_general:
        ...
        # Make the shared-team photo capability impossible to miss on mobile.
        # This is the PUBLIC/ALL-EMPLOYEES chat under Team Chat & AI, not the
        # left-sidebar direct-message chat. The actual uploader is rendered in
        # the composer below by render_order_comments().
        st.info("📷 You can attach a cake/reference picture in this shared Team Chat. Use the **Attach Picture / Camera** control below the message box.")
        render_order_comments(GENERAL_THREAD_ID, key_suffix="general", title="💬 Team Chat — All Employees", expanded=True)
...
def _looks_like_supported_phone_image(uploaded_file):
    """Validate a phone image without relying on Safari's filename metadata.

    Returns (ok, reason). We accept normal JPEG/PNG plus iPhone HEIC/HEIF by
    extension, MIME type, or common file signatures. This validation runs only
    after Streamlit has accepted the upload, preventing the browser-side red
    caution caused by a strict `type=` filter.
    """
    try:
        name = str(getattr(uploaded_file, "name", "") or "")
        suffix = Path(name).suffix.lower()
        mime = str(getattr(uploaded_file, "type", "") or "").lower().strip()
        allowed_suffixes = {".jpg", ".jpeg", ".png", ".heic", ".heif"}
        if suffix in allowed_suffixes:
            return True, ""
        if mime.startswith("image/"):
            # iOS can report image/heic, image/heif, image/jpeg, image/png, or a
            # vendor-ish image MIME while omitting a useful extension.
            return True, ""
        ...
        if head.startswith(b"\x89PNG\r\n\x1a\n"):
            return True, ""  # PNG
        # HEIF-family files are ISO-BMFF containers and normally expose an ftyp
        # brand such as heic/heix/hevc/hevx/mif1/msf1 near the beginning.
        if len(head) >= 12 and head[4:8] == b"ftyp":
            brand = head[8:12].lower()
            if brand in {b"heic", b"heix", b"hevc", b"hevx", b"mif1", b"msf1"}:
                return True, ""
        return False, "The selected file does not look like a JPG, PNG, HEIC or HEIF image."
    except Exception as exc:
        return False, f"Could not inspect the selected image: {exc}"
...
def render_customer_care():
    ...
    if submitted:
        ...
        invalid_images = []
        for img in (imgs or []):
            ok_img, reason_img = _looks_like_supported_phone_image(img)
            if not ok_img:
                invalid_images.append(f"{getattr(img, 'name', 'image')}: {reason_img}")
        if invalid_images:
            st.error("These selected files do not look like supported photos: " + " | ".join(invalid_images) + " Please choose the image again from Photos or Files.")
            return
...
def render_daily_material_accountability(department, staff_label="Staff member", key_prefix="daily_mat"):
    """Fast end-of-day material accountability for stages where per-cake logging is optional.

    This records the total quantity used during the shift rather than blocking production
    on every cake. HODs can review the full team's entries for a selected date.
    """
    st.markdown("### 📋 End-of-Day Material Accountability")
    st.caption(
        "Record the day's total material usage once at the end of the shift. "
        "Per-cake material logging remains available for unusual/high-value items, but it does not block the job workflow."
    )
    person = st.text_input(
        staff_label, value=st.session_state.get("staff_name", ""), key=f"{key_prefix}_person")
    acc_date = st.date_input("Date", value=date.today(), key=f"{key_prefix}_date")
    a, b, c = st.columns(3)
    options = STAGE_MATERIALS.get(department, ["Other"])
    item = a.selectbox("Material", options, key=f"{key_prefix}_item")
    if item == "Other":
        item = a.text_input("Specify material", key=f"{key_prefix}_item_other")
    qty = b.number_input("Total used today", min_value=0.0, step=1.0, key=f"{key_prefix}_qty")
    unit = c.selectbox("Unit", ["pieces", "kg", "grams", "litres", "ml", "trays", "boxes", "rolls"],
                       key=f"{key_prefix}_unit")
    notes = st.text_input("Notes (optional)", placeholder="e.g. mostly red fondant / 3 wedding cakes",
                          key=f"{key_prefix}_notes")
    if st.button("➕ Add Material to Today's Total", key=f"{key_prefix}_add", width='stretch'):
        if not str(person).strip():
            st.error(f"Enter the {staff_label.lower()} name.")
        elif not str(item).strip() or qty <= 0:
            st.error("Choose a material and enter a quantity greater than zero.")
        else:
            with connect() as conn:
                conn.execute("""INSERT INTO daily_material_accountability(
                    staff_name, department, accountability_date, item_name, quantity_used, unit, notes, recorded_at)
                    VALUES(?,?,?,?,?,?,?,?)""",
                    (str(person).strip(), department, str(acc_date), str(item).strip(), float(qty), unit, notes, now_iso()))
                conn.commit()
            st.success(f"Saved: {qty:g} {unit} of {item}.")
            st.rerun()

    records = load_table("daily_material_accountability")
    if not records.empty:
        mine = records[(records["department"] == department) &
                       (records["staff_name"] == str(person).strip()) &
                       (records["accountability_date"] == str(acc_date))]
    else:
        mine = records
    st.markdown(f"#### {first_name(person) if person else 'Your'} totals for {acc_date.strftime('%d %b %Y')}")
    table(mine.sort_values("recorded_at", ascending=False) if not mine.empty else mine,
          ["item_name", "quantity_used", "unit", "notes", "recorded_at"])

    if st.session_state.get("is_hod"):
        st.markdown("#### 👑 HOD view — department totals")
        hod_date = st.date_input("Day to review", value=date.today(), key=f"{key_prefix}_hod_date")
        all_day = records[(records["department"] == department) &
                          (records["accountability_date"] == str(hod_date))] if not records.empty else records
        table(all_day.sort_values(["staff_name", "recorded_at"]) if not all_day.empty else all_day,
              ["staff_name", "item_name", "quantity_used", "unit", "notes", "recorded_at"])
...
def render_decoration():
    ...
    if view_mode == "📷 Simple View":
        render_simple_view_live("Decoration", "decorator_assigned", "Decorator", "Decorating Incoming", "Decorating",
                            ...
                            "Decoration", materials_required=False)
        return
    df = load_orders()
    render_hod_overview("Decoration", df)
    t0,t1,t3,t4,t_eod,t5,t6 = st.tabs(["📅 Incoming Workload", "Incoming from Covering", "Decorating", "Correction Required", "📋 End-of-Day Materials", "✅ Finished Work", "🖼️ All Orders"])
    with t0:
    ...
    with t1:
        ...
        if row is not None:
            ...
            render_stage_material_planning("Decoration", row, row.get("decorator_assigned"))
            st.caption("Material logging on this cake is optional. Decorators can record the day's totals later in 📋 End-of-Day Materials.")
            ...
            a,b = st.columns(2)
            if a.button("✅ Covering Accepted → Start Decorating", width='stretch', disabled=not may_act):
    ...
    with t3:
        ...
        if row is not None:
            ...
            render_stage_material_planning("Decoration", row, row.get("decorator_assigned"))
            st.caption("Per-cake materials are optional. Use 📋 End-of-Day Materials before leaving your shift to account for totals used today.")
    ...
    with t_eod:
        render_daily_material_accountability("Decoration", staff_label="Decorator name", key_prefix="deco_daily_mat")
