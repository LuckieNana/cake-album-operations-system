...
APP_VERSION = "v2.8 — Release 17.0 (Mobile Chat Photos + iPhone Video Uploads)"
...
def _prepare_uploaded_reference_video(uploaded_file, order_id, index):
    """Save an iPhone/Android video immediately without putting it in SQLite/base64.

    The upload path mirrors our successful iPhone-photo approach: accept the native phone
    file first and keep Customer Care moving. MOV/M4V clips can be converted to H.264 MP4
    in a background thread after the order is saved, so video processing never blocks the
    Create Order button.
    """
    ...
    with raw_path.open("wb") as fh:
        shutil.copyfileobj(uploaded_file, fh, length=1024 * 1024)
    uploaded_file.seek(0)
    mime = {
        ".mp4": "video/mp4", ".mov": "video/quicktime", ".m4v": "video/x-m4v",
        ".webm": "video/webm", ".3gp": "video/3gpp", ".hevc": "video/mp4",
    }.get(suffix, content_type or "video/mp4")
    return str(raw_path), mime, raw_path.stat().st_size
...
def render_customer_care():
    ...
    with st.form(f"new_order_form_{order_form_gen}", clear_on_submit=False):
        ...
        vids = st.file_uploader("Customer Reference Video(s)", type=["mp4","mov","webm","m4v","3gp","hevc"], accept_multiple_files=True,
                                 help="iPhone and Android videos are accepted (MOV/MP4/M4V/WebM/3GP). The upload is saved immediately; iPhone MOV clips are optimized in the background when possible. Maximum 150MB per video.")
